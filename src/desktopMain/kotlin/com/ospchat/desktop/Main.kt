...
import com.ospchat.desktop.ui.CallStatusBar
...
private fun MainRoot(
    ...
    // Hoisted out of MainShell so it survives navigation into Chat / GroupChat /
    // InCall — those screens still render with the persistent NavigationRail
    // visible on the left, and the rail click handler below pops back to
    // Screen.Main without losing which tab the user was on.
    var selectedTab by remember { mutableStateOf(Tab.Contacts) }
    var infoPeer by remember { mutableStateOf<PeerRecord?>(null) }
    var showCreateGroup by remember { mutableStateOf(false) }
    val uiScope = rememberCoroutineScope()

    // Observed once and reused below: the global call-status banner (shown
    // above every screen but Screen.InCall) and the incoming-call dialog
    // both filter off the same activeCall row.
    val activeCall by controller.container.callRepository.activeCall
        .collectAsState(initial = null)
    val barCall =
        activeCall?.takeIf { call ->
            screen !is Screen.InCall &&
                !(call.direction == Call.Direction.INCOMING && call.state == Call.State.RINGING)
        }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        barCall?.let { call ->
            CallStatusBar(
                call = call,
                onClick = { screen = Screen.InCall(call.id) },
                onHangUp = { controller.hangUp(call.id) },
            )
        }
        Box(modifier = Modifier.fillMaxSize().weight(1f)) {
            MainShell(
                selectedTab = selectedTab,
                // Persistent left rail. From any sub-screen (Chat / GroupChat /
                // InCall) clicking a rail item pops back to the main shell with
                // the chosen tab, so the rail doubles as the "exit this
                // sub-screen" affordance — and on InCall specifically it
                // reveals the CallStatusBar that was suppressed while the
                // full-screen call UI was active.
                onTabClick = { tab ->
                    selectedTab = tab
                    screen = Screen.Main
                },
                ...
                when (val current = screen) {
                    ...
                    is Screen.InCall -> {
                        ...
                        if (call != null && call.id == current.callId) {
                            // No auto-collapse to Main on CONNECTED — the user
                            // drives this with the persistent NavigationRail:
                            // clicking any tab pops back to the shell (and
                            // reveals the CallStatusBar), clicking the bar
                            // restores this full-screen UI. Hangup / remote
                            // teardown still pop via the branches below.
                            CallScreen(
                                call = call,
